...
import {
  ...
  discordAllowedMentionsForUsers,
...
export async function POST(request: Request) {
  ...
  if (!decision.allowed) {
    if (replyTarget._type === "discord:Channel") {
      // Tell the user in-channel so they aren't left wondering, then 429 the caller.
      await postDiscordChannelMessage(replyTarget.channelId, decision.reason, {
        messageId: replyTarget.messageId,
        allowedMentions: discordAllowedMentionsForUsers([invokerUserId]),
      });
    }
    return NextResponse.json(
      { error: decision.reason, kind: "denied" },
      { status: 429 }
    );
  }
  ...
  if (replyTarget._type === "discord:Channel") {
    ...
    const mentionOpts = { allowedMentions: discordAllowedMentionsForUsers([invokerUserId]) };
