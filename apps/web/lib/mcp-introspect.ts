...
function getMcpEndpoint(): string {
  // Prefer an explicit endpoint, fall back to MCP_BASE_URL + /api/mcp.
...
export async function introspectMcp(): Promise<McpSnapshot> {
  ...
  if (!endpoint) return { ...empty, error: "MCP_BASE_URL not set" };
  if (!token) return { ...empty, error: "AGENT_Z_MCP_TOKEN not set" };
