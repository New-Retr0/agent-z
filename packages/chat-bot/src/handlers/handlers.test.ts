...
vi.mock("../invoke-verify", () => ({ invokeVerify: mocks.invokeVerify }));
...
describe("chat-bot handlers", () => {
  beforeEach(() => {
    ...
    mocks.invokeVerify.mockResolvedValue({ kind: "skipped-emoji" });
  });

  it("answers slash help without invoking the agent", async () => {
  ...
  it("keeps DMs guild-only and does not invoke the agent", async () => {
  ...
  it("does not mutate on reactions", async () => {
    ...
    onReaction({ onReaction: vi.fn((handler) => handlers.push(handler)) } as any);

    await handlers[0]({ added: true, user: { userId: "user-1", isBot: false }, rawEmoji: "✅", messageId: "12345", threadId: "discord:guild:channel", });

    expect(mocks.invokeVerify).toHaveBeenCalled();
  });
