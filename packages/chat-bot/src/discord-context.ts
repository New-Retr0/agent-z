...
export function getSlashDiscordContext(event: SlashCommandEvent): DiscordInvocationContext {
  ...
  const userMember = asRecord(member?.user);
  const userSlash = asRecord(raw?.user);
  const invoker = userMember ?? userSlash;
  return {
    ...
    invokerUserId: stringValue(userMember?.id) || stringValue(userSlash?.id),
    invokerUsername: stringValue(invoker?.username),
    invokerGlobalName: stringValue(invoker?.global_name),
  };
}
...
export function getMentionDiscordContext(thread: Thread, message: Message): DiscordInvocationContext {
  ...
  return {
    ...
    invokerUserId: stringValue(author?.id),
    invokerUsername: stringValue(author?.username),
    invokerGlobalName: stringValue(author?.global_name),
  };
}
