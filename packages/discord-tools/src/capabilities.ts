...
export function effectiveAllowedGuildIds(envAllowed: Set<string> | null, expectedGuildId?: string): Set<string> | null {
  if (envAllowed?.size) {
    return envAllowed;
  }
...
export async function assertChannelIdAllowed(
  config: DiscordToolConfig,
  channelId: string,
  allowedGuildIds: Set<string> | null,
  context: DiscordCapabilityContext
...
export function assertExpectedGuild(actualGuildId: string, expectedGuildId?: string) {
...
export function assertSnowflake(name: string, value: string) {
  if (!SNOWFLAKE_RE.test(value)) {
    throw new Error(`${name} must be a Discord snowflake.`);
  }
}

export function buildAuditReason(context: DiscordCapabilityContext, action: string, reason?: string) {
