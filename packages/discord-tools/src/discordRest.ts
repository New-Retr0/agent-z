...
export async function discordRequest(
  ...
  options: {
    ...
    query?: QueryRecord;
    body?: unknown;
    /** Multipart body (e.g. emoji create). Do not pass `body` when using this. */
    formData?: FormData;
    auditReason?: string;
  }
): Promise<DiscordRequestResult> {
  const { method, path, query, body, formData, auditReason } = options;
  if (!path.startsWith("/")) {
  ...
  const useForm = Boolean(formData);
  const hasJsonBody = !useForm && body !== undefined && body !== null && method !== "GET" && method !== "DELETE";
  if (hasJsonBody) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    const response = await fetch(url, {
      method,
      headers: baseHeaders,
      body: useForm ? formData : hasJsonBody ? JSON.stringify(body) : undefined,
    });
